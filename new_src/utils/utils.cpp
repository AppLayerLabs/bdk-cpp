...
void Utils::logToDebug(std::string pfx, std::string func, std::string data) {
  debug_mutex.lock();
...
std::string Utils::randBytes(const size_t &size) {
...
uint256_t Utils::bytesToUint256(const std::string_view &b) {
  ...
  uint256_t ret;
  boost::multiprecision::import_bits(ret, b.begin(), bytes.end(), 8);
  return ret;
}
...
int Utils::hexCharToInt(char c) noexcept {
...
bool Utils::isAddress(const std::string& add, const bool fromRPC) {
  if (fromRPC) {
    ...
    if (!std::regex_match(add, addRegex)) {
      return false;
    } else if (std::regex_match(add, lowRegex) || std::regex_match(add, uppRegex)) {
      return true;
    } else {
    return add.size() == 20;
    }
