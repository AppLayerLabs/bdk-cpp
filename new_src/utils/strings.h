...
public:
  ...
  explicit FixedStr(const std::string_view& data) {
    this->data = data;
    this->data.resize(N);
  } ///< Copy constructor.

  explicit FixedStr(std::string&& data) {
  ...
  FixedStr(FixedStr&& other) noexcept {
   ...
   */
  const std::string_view view(const size_t& size = N, const size_t& offset = 0) const {
    std::string_view ret;
    ret = this->data.substr(offset, size);
    return ret;
  }
  ...
  const std::string::const_iterator cbegin() const { return this->data.cbegin(); }

  /// Get an iterator poiinting to the end of the data string.
  const std::string::const_iterator cend() const { return this->data.cend(); }
...
public:
  ...
  Address(Address&& other) : FixedStr<20>(std::move(other.data)) {};
};
