...
sed -i '/\/\/ .tendermint.consensus.Vote vote = 6;/,+1s|bool has_vote() const;|bool MANGLED_has_vote() const;|' "$HEADER_FILE"

# Second substitution: Replace `bool _internal_has_vote() const;` with `bool _internal_MANGLED_has_vote() const;`
sed -i '/\/\/ .tendermint.consensus.Vote vote = 6;/,+3s|bool _internal_has_vote() const;|bool _internal_MANGLED_has_vote() const;|' "$HEADER_FILE"
...
sed -i '/\/\/ .tendermint.consensus.Vote vote = 6;/,+5s|inline bool Message::_internal_has_vote() const {|inline bool Message::_internal_MANGLED_has_vote() const {|' "$HEADER_FILE"

# Fourth substitution: Replace `return _internal_has_vote();` with `return MANGLED_internal_has_vote();`
sed -i '/\/\/ .tendermint.consensus.Vote vote = 6;/,+6s|return _internal_has_vote();|return _internal_MANGLED_has_vote();|' "$HEADER_FILE"

# Fifth substitution: Replace `inline bool Message::has_vote() const {` with `inline bool Message::MANGLED_has_vote() const {`
sed -i '/\/\/ .tendermint.consensus.Vote vote = 6;/,+7s|inline bool Message::has_vote() const {|inline bool Message::MANGLED_has_vote() const {|' "$HEADER_FILE"
...
# 
sed -i '/inline const ::tendermint::consensus::v1::Vote& Message::_internal_vote() const {/,+1s|return _internal_has_vote()|return _internal_MANGLED_has_vote()|' "$HEADER_FILE"
...
sed -i '/\/\/ .tendermint.consensus.Vote vote = 6;/,+1s|_internal_has_vote()|_internal_MANGLED_has_vote()|' "$CC_FILE"
