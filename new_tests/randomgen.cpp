...
#include "../new_src/utils/randomgen.h"
...
namespace TRandomGen {
  TEST_CASE("RandomGen Class") {
    ...
    SECTION("RandomGen getSeed Test") {
    ...
    SECTION("RandomGen randomness Test") {
      Hash seed = Hash::random();
      RandomGen generator(seed);

      std::vector<uint256_t> randoms;
      for (int i = 0; i < 10000; i++) {
        randoms.push_back(generator());
      }
