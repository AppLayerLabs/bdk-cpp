...
/**
 * Safe wrapper for a uint64 variable.
 * Used to safely store a uint64 within a contract.
 * @see SafeBase
 */
class SafeUint64_t : public SafeBase {
  private:
    uint64_t value; ///< Value.
    mutable std::unique_ptr<uint64_t> valuePtr; ///< Pointer to the value. check() requires this to be mutable.

    /// Check if the pointer is initialized (and initialize it if not).
    inline void check() const override {
      if (valuePtr == nullptr) valuePtr = std::make_unique<uint64_t>(value);
    };
  ...
  public:
    ...
    SafeUint64_t(DynamicContract *owner, const uint64_t& value = 0)
      : SafeBase(owner), value(0), valuePtr(std::make_unique<uint64_t>(value))
    {};
    ...
    SafeUint64_t(const uint64_t& value = 0)
      : SafeBase(nullptr), value(0), valuePtr(std::make_unique<uint64_t>(value))
    {};

    /// Copy constructor.
    SafeUint64_t(const SafeUint64_t& other) : SafeBase(nullptr) {
      other.check(); value = 0; valuePtr = std::make_unique<uint64_t>(*other.valuePtr);
    };
    ...
    inline SafeUint64_t operator&(const SafeUint64_t& other) const {
      check(); return SafeUint64_t(*valuePtr & other.get());
    };

    /// Bitwise AND operator.
    inline SafeUint64_t operator&(const uint64_t& other) const {
      check(); return SafeUint64_t(*valuePtr & other);
    };
