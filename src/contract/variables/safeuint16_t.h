...
/**
 * Safe wrapper for a uint16 variable.
 * Used to safely store a uint16 within a contract.
 * @see SafeBase
 */
class SafeUint16_t : public SafeBase {
  private:
    uint16_t value; ///< Value.
    mutable std::unique_ptr<uint16_t> valuePtr; ///< Pointer to the value. check() requires this to be mutable.

    /// Check if the pointer is initialized (and initialize it if not).
    inline void check() const override {
      if (valuePtr == nullptr) valuePtr = std::make_unique<uint16_t>(value);
    };
  ...
  public:
    ...
    SafeUint16_t(DynamicContract *owner, const uint16_t& value = 0)
      : SafeBase(owner), value(0), valuePtr(std::make_unique<uint16_t>(value))
    {};
    ...
    SafeUint16_t(const uint16_t& value = 0)
      : SafeBase(nullptr), value(0), valuePtr(std::make_unique<uint16_t>(value))
    {};

    /// Copy constructor.
    SafeUint16_t(const SafeUint16_t& other) : SafeBase(nullptr) {
      other.check(); value = 0; valuePtr = std::make_unique<uint16_t>(*other.valuePtr);
    };
    ...
    inline SafeUint16_t operator&(const SafeUint16_t& other) const {
      check(); return SafeUint16_t(*valuePtr & other.get());
    };

    /// Bitwise AND operator.
    inline SafeUint16_t operator&(const uint16_t& other) const {
      check(); return SafeUint16_t(*valuePtr & other);
    };
