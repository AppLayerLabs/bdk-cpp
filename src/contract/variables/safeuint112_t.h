#ifndef SAFEUINT112_T_H
#define SAFEUINT112_T_H
...
/**
 * Safe wrapper for a uint112 variable.
 * Used to safely store a uint112 within a contract.
 * @see SafeBase
 */
class SafeUint112_t : public SafeBase {
  private:
    uint112_t value; ///< Value.
    mutable std::unique_ptr<uint112_t> valuePtr; ///< Pointer to the value. check() requires this to be mutable.

    /// Check if the pointer is initialized (and initialize it if not).
    inline void check() const override {
      if (valuePtr == nullptr) valuePtr = std::make_unique<uint112_t>(value);
    };
  ...
  public:
     ...
     */
    SafeUint112_t(DynamicContract* owner, const uint112_t& value = 0)
      : SafeBase(owner), value(0), valuePtr(std::make_unique<uint112_t>(value))
    {};
     ...
     * @param value The initial value. Defaults to 0.
     */
    SafeUint112_t(const uint112_t& value = 0)
      : SafeBase(nullptr), value(0), valuePtr(std::make_unique<uint112_t>(value))
    {};

    /// Copy constructor.
    SafeUint112_t(const SafeUint112_t& other) : SafeBase(nullptr) {
      other.check(); value = 0; valuePtr = std::make_unique<uint112_t>(*other.valuePtr);
    };

    /// Getter for the value. Returns the value from the pointer.
    inline uint112_t get() const { check(); return *valuePtr; };
     ...
     */
    inline SafeUint112_t operator+(const SafeUint112_t& other) const {
      check();
      if (*valuePtr > std::numeric_limits<uint112_t>::max() - other.get()) {
        throw std::overflow_error("Overflow in addition operation");
      }
      return SafeUint112_t(*valuePtr + other.get());
    };
     ...
     * @throw std::overflow_error if an overflow happens.
     */
    inline SafeUint112_t operator+(const uint112_t& other) const {
      check();
      if (*valuePtr > std::numeric_limits<uint112_t>::max() - other) {
        throw std::overflow_error("Overflow in addition operation");
      }
      return SafeUint112_t(*valuePtr + other);
    };
     ...
     * @throw std::overflow_error if an overflow happens.
     */
    inline SafeUint112_t operator+(const uint64_t& other) const {
      check();
      if (*valuePtr > std::numeric_limits<uint112_t>::max() - other) {
        throw std::overflow_error("Overflow in addition operation");
      }
    ...
    inline SafeUint112_t operator&(const SafeUint112_t& other) const {
      check(); return SafeUint112_t(*valuePtr & other.get());
    };

    /// Bitwise AND operator.
    inline SafeUint112_t operator&(const uint112_t& other) const {
      check(); return SafeUint112_t(*valuePtr & other);
    };
    ...
    inline bool operator&&(const SafeUint112_t& other) const {
      check(); return *valuePtr && other.get();
    };

    /// Logical AND operator.
    inline bool operator&&(const uint112_t& other) const {
    ...
    /// Assignment operator.
    inline SafeUint112_t& operator=(const SafeUint112_t& other) {
      check(); markAsUsed(); *valuePtr = other.get(); return *this;
    };

    /// Assignment operator.
    inline SafeUint112_t& operator=(const uint112_t& other) {
      check(); markAsUsed(); *valuePtr = other; return *this;
    };

    /// Assignment operator.
    inline SafeUint112_t& operator=(const uint64_t& other) {
      check(); markAsUsed(); *valuePtr = other; return *this;
    };
     ...
     * @throw std::overflow_error if an overflow happens.
     */
    inline SafeUint112_t& operator++() {
      check();
      markAsUsed();
      if (*valuePtr == std::numeric_limits<uint112_t>::max()) {
    ...
    inline SafeUint112_t& operator--() {
      check();
      markAsUsed();
      if (*valuePtr == std::numeric_limits<uint112_t>::min()) {
