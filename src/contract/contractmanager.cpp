...
ContractManager::ContractManager(State *state, const std::unique_ptr<DB> &db,
  ...
  for (const auto &contract : contracts) {
    ...
    if (contract.value == "SimpleContract") {
      Address contractAddress(contract.key, true);
      this->contracts.insert(std::make_pair(
          contractAddress, std::make_unique<SimpleContract>(
...
void ContractManager::createNewSimpleContractContract(
  ...
  this->contracts.insert(std::make_pair(
      ...
      std::make_unique<SimpleContract>(
          decoder.getData<std::string>(0), decoder.getData<uint256_t>(1),
          this->interface, derivedContractAddress, this->getCaller(),
          this->options->getChainID(), this->db)));
}

void ContractManager::validateCreateNewSimpleContractContract(
...
void ContractManager::createNewERC20Contract(const ethCallInfo &callInfo) {
  ...
  this->contracts.insert(std::make_pair(
      derivedContractAddress,
      std::make_unique<ERC20>(this->interface, decoder.getData<std::string>(0),
...
void ContractManager::createNewERC20NativeWrapperContract(
  ...
  this->contracts.insert(std::make_pair(
      ...
      std::make_unique<NativeWrapper>(
          this->interface, decoder.getData<std::string>(0),
          decoder.getData<std::string>(1),
          uint8_t(decoder.getData<uint256_t>(2)), derivedContractAddress,
          this->getCaller(), this->options->getChainID(), this->db)));
}
...
void ContractManager::ethCall(const ethCallInfo &callInfo) {
  ...
  if (this->getCommit()) {
    ...
    const std::string ContractManager::ethCallView(const ethCallInfo &data)
        const {
    ...
    bool ContractManager::validateCallContractWithTx(
        const ethCallInfo &callInfo) {
    ...
    const std::string ContractManager::callContract(const ethCallInfo &callInfo)
        const {
      const auto &[from, to, gasLimit, gasPrice, value, data] = callInfo;
      if (to == this->getContractAddress()) {
    ...
    std::vector<std::pair<std::string, Address>> ContractManager::getContracts()
        const {
      std::shared_lock lock(this->contractsMutex);
