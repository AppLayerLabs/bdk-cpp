...
DEXV2Factory::DEXV2Factory(ContractManagerInterface &interface, const Address &address, const std::unique_ptr<DB> &db) :
  DynamicContract(interface, address, db), feeTo_(this), feeToSetter_(this), allPairs_(this), getPair_(this)
{
  ...
  auto allPairs = db->getBatch(this->getNewPrefix("allPairs_"));
  for (const auto& dbEntry : allPairs) {
    this->allPairs_.push_back(Address(dbEntry.value));
  }
  auto getPairs = db->getBatch(this->getNewPrefix("getPair_"));
...
) : DynamicContract(interface, "DEXV2Factory", address, creator, chainId, db), feeTo_(this), feeToSetter_(this), allPairs_(this), getPair_(this)
{
...
DEXV2Factory::~DEXV2Factory() {
  ...
  for (const auto& address : this->allPairs_.get()) {
    batchOperations.push_back(Utils::uint32ToBytes(index), address.view_const(), this->getNewPrefix("allPairs_"));
  }
...
Address DEXV2Factory::feeTo() const {
  return this->feeTo_.get();
}

Address DEXV2Factory::feeToSetter() const {
  return this->feeToSetter_.get();
}

std::vector<Address> DEXV2Factory::allPairs() const {
  return this->allPairs_.get();
}

uint64_t DEXV2Factory::allPairsLength() const {
  return this->allPairs_.size();
}
...
void DEXV2Factory::setFeeTo(const Address& feeTo) {
  this->feeTo_ = feeTo;
}

void DEXV2Factory::setFeeToSetter(const Address& feeToSetter) {
  this->feeToSetter_ = feeToSetter;
}
