...
/**
 * Variant of SnailTracer that uses int192 instead of int256.
 * Any number lower than that will cause a SIGFPE (division by zero) during
 * calls to norm() due to under/overflows in the original contract's math logic.
 */
class SnailTracerOptimized : public DynamicContract {
  private:
    ...
    using SVector = SafeTuple<int192_t, int192_t, int192_t>; ///< SafeVar version of Vector.
    using SRay = SafeTuple<Vector, Vector, int192_t, bool>; ///< SafeVar version of Ray.
    ...
    SafeInt192_t width_; ///< Width of the image to be generated (fixed for life).
    SafeInt192_t height_; ///< Height of the image to be generated (fixed for life).
  ...
  public:
    ...
    using ConstructorArguments = std::tuple<int192_t, int192_t>;
     ...
     */
    std::tuple<uint8_t, uint8_t, uint8_t> TracePixel(const int192_t& x, const int192_t& y, const uint192_t& spp);
     ...
     */
    Bytes TraceScanline(const int192_t& y, const int192_t& spp);
     ...
     */
    Bytes TraceImage(const int192_t& spp);
     ...
     */
    Vector trace(const int192_t& x, const int192_t& y, const int192_t& spp);
     ...
     */
    int192_t clamp(const int192_t& x);
     ...
     */
    int192_t sqrt(const int192_t& x);
     ...
     */
    Vector mul(const Vector& v, const int192_t& m);
     ...
     */
    Vector div(const Vector& v, const int192_t& d);
     ...
     */
    int192_t dot(const Vector& u, const Vector& v);
     ...
     */
    int192_t intersect(const Sphere& s, const Ray& r);
     ...
     * @return The distance until the first intersection point, or zero in case of no intersection.
     */
    int192_t intersect(const Triangle& t, const Ray& r);
     ...
     * @return A vector representing the radiance.
     */
    Vector radiance(const Ray& ray, const Sphere& obj, const int192_t& dist);
     ...
     * @return A vector representing the radiance.
     */
    Vector radiance(const Ray& ray, const Triangle& obj, const int192_t& dist);
     ...
     */
    std::tuple<int192_t, Primitive, uint192_t> traceray(const Ray& ray);

    /// Register the contract structure.
    static void registerContract() {
      ContractReflectionInterface::registerContractMethods<
        SnailTracerOptimized, int192_t&, int192_t&,
        const Address&, const Address&, const uint192_t&, DB&
      >(
        ...
        std::make_tuple("clamp", static_cast<int192_t(SnailTracerOptimized::*)(const int192_t&)>(&SnailTracerOptimized::clamp), FunctionTypes::NonPayable, std::vector<std::string>{"x"}),
        ...
        std::make_tuple("mul", static_cast<Vector(SnailTracerOptimized::*)(const Vector&,const int192_t&)>(&SnailTracerOptimized::mul), FunctionTypes::NonPayable, std::vector<std::string>{"v", "m"}),
        ...
        std::make_tuple("intersect", static_cast<int192_t(SnailTracerOptimized::*)(const Sphere&, const Ray&)>(&SnailTracerOptimized::intersect), FunctionTypes::NonPayable, std::vector<std::string>{"s", "r"}),
        std::make_tuple("intersect", static_cast<int192_t(SnailTracerOptimized::*)(const Triangle&, const Ray&)>(&SnailTracerOptimized::intersect), FunctionTypes::NonPayable, std::vector<std::string>{"t", "r"}),
        ...
        std::make_tuple("radiance", static_cast<Vector(SnailTracerOptimized::*)(const Ray&, const Sphere&, const int192_t&)>(&SnailTracerOptimized::radiance), FunctionTypes::NonPayable, std::vector<std::string>{"ray", "obj", "dist"}),
        std::make_tuple("radiance", static_cast<Vector(SnailTracerOptimized::*)(const Ray&, const Triangle&, const int192_t&)>(&SnailTracerOptimized::radiance), FunctionTypes::NonPayable, std::vector<std::string>{"ray", "obj", "dist"}),
