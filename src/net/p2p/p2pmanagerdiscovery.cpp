...
namespace P2P {
    ...
    switch (message.type()) {
      ...
      default:
        Utils::logToDebug(Log::P2PParser, __func__,
          "Invalid message type from " + session->hostNodeId().hex().get() + ", closing session."
        );
  ...
  void ManagerDiscovery::handleRequest(
    ...
    switch (message.command()) {
      case Ping:
        ...
        break;
      //case Info:
        // We don't handle info within discovery because
        // discovery keeps zero track of blockchain history.
      ...
      default:
        Utils::logToDebug(Log::P2PParser, __func__,
          "Invalid Request Command Type from " + session->hostNodeId().hex().get() + ", closing session."
        );
        this->disconnectSession(session->hostNodeId());
        break;
    }
  ...
  void ManagerDiscovery::handleAnswer(
    ...
    switch (message.command()) {
      ...
      default:
        Utils::logToDebug(Log::P2PParser, __func__,
          "Invalid Answer Command Type from " + session->hostNodeId().hex().get() + ", closing session."
        );
        this->disconnectSession(session->hostNodeId());
        break;
    }
  ...
  void ManagerDiscovery::handlePingRequest(
    ...
    if (!RequestDecoder::ping(message)) {
      Utils::logToDebug(Log::P2PParser, __func__,
        "Invalid ping request from " + session->hostNodeId().hex().get() + " closing session."
      );
      this->disconnectSession(session->hostNodeId());
      return;
    }
