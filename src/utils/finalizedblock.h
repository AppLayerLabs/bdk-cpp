...
class FinalizedBlock {
  ...
  public:
    ...
    FinalizedBlock(
      ...
      Hash&& hash//,
      //size_t size
    ) : //validatorSig_(std::move(validatorSig)),
    proposerAddr_(std::move(proposerAddr)), 
    //validatorPubKey_(std::move(validatorPubKey)),
      prevBlockHash_(std::move(prevBlockHash)), //blockRandomness_(std::move(blockRandomness)),
      //validatorMerkleRoot_(std::move(validatorMerkleRoot)),
      txMerkleRoot_(std::move(txMerkleRoot)),
      timestamp_(timestamp), nHeight_(nHeight),
      //txValidators_(std::move(txValidators)), 
      txs_(std::move(txs)), hash_(std::move(hash))//, size_(size)
