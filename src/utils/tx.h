...
private:
  ...
  uint256_t maxPriorityFeePerGas_; ///< Max priority fee per gas, as per
                                   ///< [EIP-1559](https://eips.ethereum.org/EIPS/eip-1559),
                                   ///< in Wei.
  uint256_t maxFeePerGas_; ///< Max fee per gas, as per
                           ///< [EIP-1559](https://eips.ethereum.org/EIPS/eip-1559),
                           ///< in Wei.
...
public:
  ...
  inline const uint256_t &getMaxPriorityFeePerGas() const {
    return this->maxPriorityFeePerGas_;
  }

  /// Getter for `maxPerGas`.
  inline const uint256_t &getMaxFeePerGas() const {
...
public:
   ...
   * @throw std::runtime_error on signing failure or sender mismatch.
   */
  TxValidator(const Address from, const Bytes &data, const uint64_t chainId,
  ...
  /// Equality operator. Checks if both transaction hashes are equal.
  bool operator==(const TxValidator &tx) const {
