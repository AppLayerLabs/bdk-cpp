...
uint256_t EVMCConv::evmcUint256ToUint256(const evmc::uint256be& i) {
  // We can use the uint256ToBytes directly as it is std::span and we can create a span from an array
  return UintConv::bytesToUint256(bytes::View(i.bytes, 32));
}

evmc::uint256be EVMCConv::uint256ToEvmcUint256(const uint256_t& i) {
  ...
  evmc::uint256be ret;
  BytesArr<32> bytes = UintConv::uint256ToBytes(i);
  std::copy(bytes.begin(), bytes.end(), ret.bytes);
  return ret;
}

BytesArr<32> EVMCConv::evmcUint256ToBytes(const evmc::uint256be& i) {
  BytesArr<32> ret;
  std::copy(i.bytes, i.bytes + 32, ret.begin());
  return ret;
}

evmc::uint256be EVMCConv::bytesToEvmcUint256(const bytes::View b) {
  evmc::uint256be ret;
