...
Tx::Base::Base(const std::string_view &bytes, bool fromDB) {
  ...
  uint8_t vLenght = (uint8_t(bytes[index]) >= 0x80 ? uint8_t(bytes[index]) - 0x80 : 0); 
  if (vLenght != 0) {
...
// custom implementation (no RLP abstraction) 
Tx::Validator::Validator(const std::string_view &bytes) {
  uint64_t index = 0;
  ...
  if (_v > 36) {
    ...
    throw std::runtime_error(std::string(__func__) + ": " +
      std::string("RLP: Invalid transaction signature - v is not 27 or 28, v is ") + boost::lexical_cast<std::string>(this->_v)
    );
  }
...
void Tx::Validator::sign(const PrivKey &privKey) {
  if (privKey.size() != 32) {
    throw std::runtime_error(std::string(__func__) + ": " +
      std::string("Invalid private key size - expected 32, got ") + std::to_string(privKey.size())
    );
