...
//
//  public:
//    // Take ownership of the socket on construction
...
public:
    ...
    http_session(tcp::socket&& socket,std::shared_ptr<std::string const> const& doc_root, Subnet& subnet) : stream_(std::move(socket)) , doc_root_(doc_root) , queue_(*this), subnet(subnet)
    { }
...
public:
    listener(net::io_context& ioc,tcp::endpoint endpoint,std::shared_ptr<std::string const> const& doc_root, Subnet& subnet);
