 ...
 *
 * FIXME: simplify the start/stop logic now that we don't have mutex vs. callback problems
 *FIXME:
 * and we can probably call DoStop directly instead of flagging failure
 *        OR
 * check the flag and call dostop in all methods (it's safe to do it in all of them)
 */
template <typename T>
class WebsocketRPCConnection {
  protected:
    ...
    //std::function<void()> rpcConnectionResetCallback_; ///< Function to call when an active connection is stopped.
    std::atomic<bool> rpcFailed_ = false; ///< Error flag raised by failures in I/O handlers (must be atomic).

    static std::atomic<uint64_t> rpcRequestIdCounter_; ///< The JSON-RPC request ID generator is global and shared among all RPC connections.

    //cometimpl calls rpcAsyncCall(), gets the request ID which is zero if e.g. the connection is busted
    //so NO need for the txOut_ queue anymore as you are guaranteed to get a response for it irrespective of the connection object's start/stop cycle
    // all you need to do is maintain the WebsocketRPCConnection object alive as long as the CometImpl object is alive
    //rpc calls generate the rpc request ID and dispatch them
    ...
    std::map<uint64_t, T> rpcSent_; ///< Map of JSON-RPC request id to application object that models the request (the request json is generated by the app and forgotten).
     ...
     * NOTE: The caller must have locked rpcStateMutex_.
     * @param isStartFailure If set to `true`, won't invoke rpcConnectionResetCallback_() (just cleaning up a bad start attempt).
     */
    void rpcDoStop(bool isStartFailure = false);
  ...
  public:
    ...
    /**
     * Constructor.
     * @param callback Pointer to a function to call when an active RPC connection is closed.
     */
/*  WebsocketRPCConnection(std::function<void()> callback)
      : rpcConnectionResetCallback_(std::move(callback))
    {}
*/
    /**
     * Start the persisted rpcSocket_ connection to the cometbft process_.
...
void WebsocketRPCConnection<T>::rpcHandleAsyncRead(std::shared_ptr<boost::beast::flat_buffer> buffer, boost::system::error_code ec, std::size_t) {
  ...
  if (ec) {
    rpcFailed_ = true;
    SLOGDEBUG("RPC FAILED (ec): " + ec.message());
    return;
  }
  try {
    ...
    if (jsonResponse.contains("id")) {
      ...
      SLOGDEBUG("RPC FAILED (bad JSON-RPC no 'id'): " + responseStr);
      rpcFailed_ = true;
      return;
    }
...
bool WebsocketRPCConnection<T>::rpcDoStart(int rpcPort) {
  ...
  try {
    ...
    rpcThread_ = std::thread([this]() {
      ...
      // Just make sure it is flagged as failed and the connection will be stopped
      // eventually if it isn't stopped already.
      rpcFailed_ = true;
    });
    return true;
  } catch (const std::exception& ex) {
    GLOGDEBUG("ERROR: websocket connection failed: " + std::string(ex.what()));
    rpcDoStop(true); // 'true' param: don't invoke the rpc connection closed callback; just cleaning up a false start
    return false;
  }
}

template <typename T>
void WebsocketRPCConnection<T>::rpcDoStop(bool isStartFailure) {
  SLOGDEBUG("DOSTOP1");
  if (!rpcRunning_) {
    return;
  }
  ...
  rpcIoc_.restart();
  SLOGDEBUG("DOSTOP7");
  // If just failing a start, then don't call the reset callback as that can acquire mutexes
  //if (!isStartFailure) {
    // An existing connection has been reset here, so invoke the configured RPC connection reset function
    //rpcConnectionResetCallback_();
  //}
  ...
  json rpcClosedResponse = rpcMakeInternalError("rpcConnectionReset()");
...
uint64_t WebsocketRPCConnection<T>::rpcDoAsyncCall(const std::string& method, const json& params, const T& requestData, bool retry) {
  if (!rpcWs_ && !rpcDoStart()) { // rpcDoStart() doesn't invoke the connection closed callback so it's fine to call it here
    SLOGDEBUG("DOA FAIL 1");
    return 0;
  }
  // Only check rpcFailed_ flag *after* we had a chace to try and restart a stopped connection
  // since that would transparently reset the flag and allow us to try and execute the RPC send
  if (rpcFailed_) {
    // Can't call rpcDoStop() here because that can try to acquire the txOutSentMutex_ in the
    // client code's rpc connection closed callback while it is already acquired.
    SLOGDEBUG("DOA FAIL 2");
    SLOGDEBUG("rpcFailed_ == true at rpcDoAsyncCall(), stopping");
    rpcStopConnection(); // we don't have the lock yet here so we call the public function
    // Should RETRY here?
    if (!rpcWs_ && !rpcDoStart()) {
      SLOGDEBUG("rpcDoAsyncCall FAIL TO RESTART");
      return 0;
    }
    // Connected I guess?... then continue
    SLOGDEBUG("rpcDoAsyncCall RESTARTED THE CONNECTION FOR SOME REASON"); // FIXME: this is nonsense, should not be needed, should not be failing or flagged as failed in the first place
  }
  ...
  SLOGDEBUG("DOA1");
  if (requestData != T{}) {
    SLOGDEBUG("TRACKING rpcSent_ for method: " + method);
    // Only insert in rpcSent_ if this method is NOT being used as a back-end to rpcSyncCall(),
    // which uses T{} as requestData. In that case we must not be tracking requestData in rpcSent_.
    std::scoped_lock lock(rpcSentMutex_);
    rpcSent_[requestId] = requestData;
  } else {
    SLOGDEBUG("NOT TRACKING rpcSent_ for method: " + method);
  }
  SLOGDEBUG("DOA2");
  boost::asio::post(rpcStrand_, [this, requestId, message]() {
    boost::system::error_code ec;
      SLOGDEBUG("DOA LAMBDA ENTER");
    std::unique_lock<std::mutex> stateLock(rpcStateMutex_);
    if (!rpcWs_ && !rpcDoStart()) {
      SLOGDEBUG("DOA LAMBDA FAIL 1");
      ...
      SLOGDEBUG("DOA LAMBDA FAIL 1 END");
    } else {
      SLOGDEBUG("DOA LAMBDA 2");
      rpcWs_->write(boost::asio::buffer(*message), ec);
      SLOGDEBUG("DOA LAMBDA 3");
      if (ec) {
        SLOGDEBUG("DOA LAMBDA EC 4");
        rpcFailed_ = true;
        ...
        SLOGDEBUG("DOA LAMBDA EC 5");
      }
      SLOGDEBUG("DOA LAMBDA 6 END");
    }
  });
  SLOGDEBUG("DOA END");
  return requestId;
...
uint64_t WebsocketRPCConnection<T>::rpcGetNextResponse(json& outResponse, T& outRequestData) {
  ...
  if (rpcFailed_) {
    SLOGDEBUG("rpcFailed_ == true at rpcGetNextResponse(), stopping");
    rpcStopConnection(); // we don't have the lock yet here so we call the public function
    // Should RETRY here?
    if (!rpcWs_ && !rpcDoStart()) {
      SLOGDEBUG("GETNEXT FAIL TO RESTART");
      return 0;
    }
    // Connected I guess?... then continue
    SLOGDEBUG("GETNEXT RESTARTED THE CONNECTION FOR SOME REASON");
  }
...
bool WebsocketRPCConnection<T>::rpcSyncCall(const std::string& method, const json& params, json& outResult, bool retry) {
  std::unique_lock<std::mutex> stateLock(rpcStateMutex_);
  if (rpcFailed_) {
    // Don't have to worry about txOutSentMutex_ being locked by the rpc connection
    // closed callback in this fully synchronous RPC call case.
    rpcDoStop();
  }
  if (!rpcWs_ && !rpcDoStart()) {
  ...
  try {
    ...
    if (retry) {
      stateLock.unlock();
      GLOGTRACE("Exception in rpcSyncCall(): " + std::string(ex.what()));
...
using CometRPCRequestType = std::variant<TxSendType, TxCheckType>;
...
*/
    ...
    //void rpcConnectionReset(); ///< Notification by WebsocketRPCConnection that the RPC connection was closed.
...
CometImpl::CometImpl(CometListener* listener, std::string instanceIdStr, const Options& options, bool stepMode)
  : listener_(listener), instanceIdStr_(instanceIdStr), options_(options), stepMode_(stepMode)//,
    //WebsocketRPCConnection(std::bind(&CometImpl::rpcConnectionReset, this))
{
}
...
*/
     ...
     /* SUBMITTING is the default state now so no need for this
  ...
  LOGXTRACE("Calling rpcAsyncCall()");
...
/*
void CometImpl::rpcConnectionReset() {
  // Whenever the websocket RPC connection is reset, the RPC request IDs that
  //   we have in flight become meaningless, since they won't be answered in
  //   a new connection that we happen to make (or we don't expect them to be
  //   answered--in any case, we won't ever be reusing these request IDs as
  //   the local JSON-RPC request ID generator is not reset to 1).
  // So we need to iterate over every txOutSent_ entry we have with a requestID
  //   waiting for an RPC response and toss them all to the application with
  //   a rejection answer so the application can retry them when the RPC
  //   connection is re-established.

  json rpcClosedResponse = rpcMakeInternalError("rpcConnectionReset()");

  LOGXTRACE("rpcConnectionReset() locking");
  std::scoped_lock lock(txOutSentMutex_);
  LOGXTRACE("rpcConnectionReset() locked");

  auto it = txOutSent_.begin();
  while (it != txOutSent_.end()) {
    const auto& txOutItem = it->second;
    ...
    if (txCacheSize_ > 0 && txEthHash) {
      ...
      for (int i = 0; i < 2; ++i) {
        ...
        if (it != bucket.end()) {
          CometTxStatus& txStatus = it->second;
          // Just mark it as REJECTED so it can be resent.
          // Not an actual rejection since the RPC connection was closed,
          //   but rejection or connection closed should have the same
          //   default treatment (i.e. retry).
          txStatus.height = CometTxStatusHeight::REJECTED;
          break;
        }
      }
    }
    listener_->sendTransactionResult(*tx, txTicketId, false, "", rpcClosedResponse);
    it = txOutSent_.erase(it);
  }
  LOGXTRACE("rpcConnectionReset() unlocking");
}
*/
...
void CometImpl::workerLoopInner() {
  ...
  while (!stop_) {
    ...
    LOGDEBUG("Stopping cometbft inspect RPC connection...");

    rpcStopConnection();

    LOGDEBUG("Stopping cometbft...");

    stopCometBFT();

    LOGDEBUG("Done with cometbft inspect");
    ...
    while (!stop_) {
      ...
      //
      // just pump RPC responses here, since RPC requests are made asynchronously
      ...
      LOGDEBUG("R3");
        ...
        // PUMP RESPONSES OF BOTH CALLS, THAT'S IT
        // UPDATE THE TX CACHE ACCORDINGLY WITH THE T&
        // Send the T to the callback

        // --------------------------------------------------------------------------------
        // sendTransaction send requests
        // --------------------------------------------------------------------------------
/*
      // FIXME: no longer needed, there's no txout, the send is instant
        ...
        if (!txOutEmpty) {
          ...
          // We need to acquire txOutSentMutex_ before rpcAsyncCall() so that txOutSent_ won't be cleared by
          //   rpcConnectonReset() before we add our entry to it.
          std::unique_lock<std::mutex> txOutSentLock(txOutSentMutex_);
          json stparams = { {"tx", encodedTx} };
          uint64_t requestId = rpcAsyncCall("broadcast_tx_async", stparams);
          if (requestId > 0) {
            txOutSent_[requestId] = txOutItem;
            txOutSentLock.unlock();
            LOGXTRACE("Removing txOut_ item");
            stlock.lock();
            txOut_.pop_front();
            LOGXTRACE("Removed txOut_ item");
          } else {
            txOutSentLock.unlock();
            LOGXTRACE("Error in rpcAsyncCall()");
            // When rpcAsyncCall() fails, we need to explicitly call stop on the RPC connection
            // (this triggers the rpc connection closed callback, which is fine if we are not
            // holding the txOutSentMutex_).
            rpcStopConnection();
            // If we got an RPC error, we might have bigger problems (e.g. cometbft died) so quit and re-check abciServer_ above.
            // The txOut_ item is NOT removed, since it isn't sent successfully.
            break;
          }
        }
*/

        // FIXME: merge pumping out all responses
        //        remove check send transaction sending (it's done in checkTransaction() instead )

        // --------------------------------------------------------------------------------
        // sendTransaction receive responses
        // --------------------------------------------------------------------------------

/*
        // Pop all pending RPC responses and send them to the application
        while (true) {
          // Fetch an RPC response (any one)
          json stresponse;
          uint64_t requestId;
          requestId = rpcGetNextResponse(stresponse);
          if (requestId == 0) {
            break;
          }
          LOGXTRACE("Processing RPC response");
          bool stsuccess = ! stresponse.contains("error");
          ...
          // Match the JSON-RPC response's request ID to its pending txOutSent_ (RPC request).
          auto it = txOutSent_.find(requestId);
          if (it != txOutSent_.end()) {
            // Not a deque like txOut_, so copy the element out, which allows us to release the mutex
            // while invoking the application callback (a good idea by default).
            // TODO: tx in the tuple should be a shared_ptr<Bytes>, which would avoid the copy here.
            //       or the entire entry should be a shared_ptr to the tuple.
            std::unique_lock<std::mutex> lock(txOutSentMutex_);
            const auto& txOutItem = it->second;
            uint64_t txTicketId = std::get<0>(txOutItem);
            const std::optional<Hash> txEthHash = std::get<1>(txOutItem);
            const Bytes tx = std::get<2>(txOutItem);
            ...
            // Need to reacquire the mutex and look up the element again to erase
            // (iterator 'it' is potentially invalid now)
            lock.lock();
            txOutSent_.erase(requestId);
          } else {
            // Should never happen, but if it does, there's nothing to do but discard it
            LOGDEBUG("Received unknown RPC response ID, discarding: " + std::to_string(requestId));
            continue;
          }
        }
        */
...
/*
      ...
      sending is now async inside websocketrpcconnection


        // TODO: This could also be made to use rpcAsyncCall
        //       Would use a txCheckSent_, analogous to txOutSent_
        //       And they should share the same request queue and response map if the task is abstracted (std::variant).
...
*/
        // --------------------------------------------------------------------------------
        // Process all RPC responses (both sendTransaction and checkTransaction)
        // --------------------------------------------------------------------------------

        // Fetch an RPC response (any one)
        requestId = rpcGetNextResponse(requestResponseJson, requestData);
        if (requestId == 0) {
                LOGDEBUG("R3 DONE PUMP");
        ...
        // (Assumes the various requestdata types are different...)
        std::visit([&](auto&& arg) {
        LOGDEBUG("RVIS 1");
        ...
        LOGDEBUG("RVIS 3.F");
            ...
            // Bug, unknown type. Never happens.
