...
class CometListener {
  public:
     ...
     * @param noChange Outparam; if `true`, ignores `txIds` and just forwards all transactions unmodified (doesn't affect `injectTxs`).
     * @param txIds Outparam; empty vector to be set with the indices in `txs`  to be included in the block proposal, in order.
     * @param injectTxs Outparam; optional raw txs to forcefully append to the end of the block's tx list (default: none).
     */
