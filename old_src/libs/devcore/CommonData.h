...
/// Converts a big-endian byte-stream represented on a templated collection to a templated integer value in reference.
/// @a _In will typically be either std::string or bytes.
/// @a T will typically by unsigned, u160, u256 or bigint.
template <class T, class _In>
inline void fromBigEndianRef(_In const& _bytes, T& o_ret) {
	for (auto i: _bytes)
		o_ret = (T)((o_ret << 8) | (byte)(typename std::make_unsigned<decltype(i)>::type)i);
}
